...
def generate_github_report(model_name, repo, days):
...
def generate_hn_hour_topic(model_name):
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    with gr.Tab("GitHub 项目进展"):
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_github_report, inputs=[model_name, subscription_list, days], outputs=[markdown_output, file_output])
    ...
    with gr.Tab("Hacker News 热点话题"):
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_hn_hour_topic, inputs=[model_name,], outputs=[markdown_output, file_output])
